...
class GoogleOauth2Component extends Component {
    ...
    public function init($config, $scopes, $services=null) {
        ...
        if(!empty($services) && is_array($services)):
            if(in_array('google_drive', $services)):
                $this->gdrive = new Google_DriveService($this->google);
            endif;
        endif;
    }
    ...
    ), $api_mode=false, $services=null) {
        if(!$this->google):
            $this->init($config, $scopes, $services);
        endif;
