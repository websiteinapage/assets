...
App::uses('GoogleOauth2Component', 'Controller/Component');
...
class GoogleDriveComponent extends Component {
    ...
    public function connect(\Controller $controller, $config, $api_mode=false) {
        ...
        if(!empty($_REQUEST['code'])):
            $this->authenticate($controller);
            // if it makes it here
            $this->ready = true;
            // clear code GET param
            $controller->redirect($config['redirect_url']);
        else:    
            if($this->getTokens()):
    ...
    private function authenticate(\Controller $controller) {
        if(!empty($_REQUEST['code'])):
            ...
            // $this->userinfo = $this->oauth->userinfo_v2_me->->userinfo->get();
