...
class SSUtil {
	...
	public function SSUtil() {
             ...
             */
            $this->key=SLIDESHARE_API_KEY;
            $this->secret=SLIDESHARE_API_SECRET;
            $this->apiurl=SLIDESHARE_APIURL;
            $this->apiurl_v2=SLIDESHARE_APIURL_V2;
	}
	private function get_data($call,$params,$version=1) {
		$ts=time();
		$hash=sha1($this->secret.$ts);
		try {
                    switch($version):
                        case 2:
                            $res=file_get_contents($this->apiurl_v2.$call."?api_key=$this->key&ts=$ts&hash=$hash".$params);
                            break;
        ...
        public function oembed($url, $format="json") {
            $res = file_get_contents(SLIDESHARE_OEMBED_API . "?url=$url&format=$format");
            $res = utf8_encode($res);
        ...
        public function get_favorites($user) {
            return $this->XMLtoArray($this->get_data("get_user_favorites", "&username_for=$user", 2));
        }
        
        /* Add a slideshow to the user's favorites list */
        public function add_favorite($user, $password, $slideshow_id) {
            $data=$this->XMLtoArray($this->get_data("add_favorite", "&username=$user&password=$password&slideshow_id=$slideshow_id", 2));
            return $data[0];
        }
